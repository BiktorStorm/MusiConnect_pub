...
struct CoreAudioHandler::Impl {
    ...
    // FIX 1: Pre-allocated scratch buffer — avoids heap allocation inside the
    // realtime Core Audio callback, which could stall the thread unpredictably.
    ...
    AudioBufferList* ioData)
{
    ...
    // FIX 1: Use pre-allocated scratch buffer instead of allocating a new
    // vector on every callback — heap allocation on a realtime thread causes
    // unpredictable stalls that make audio choppy and non-continuous.
    impl->inputBufferList->mBuffers[0].mData = impl->inputScratch.data();
    ...
    if (impl->captureCallback) {
        impl->captureCallback(impl->inputScratch.data(), (int)inNumberFrames);
...
bool CoreAudioHandler::init(const AudioConfig& config) {
    ...
    // FIX 1: Size the scratch buffer once here, after we know the actual
    // buffer size — this is the allocation that replaces the per-callback one.
    m_impl->inputScratch.resize(m_impl->actualBufferSize);
...
bool CoreAudioHandler::start() {
    ...
    // FIX 3: Pre-fill one frame of silence into the playout buffer before
    // starting audio. Without this the output callback fires immediately on
    // an empty buffer, causing underruns and gaps until the first decoded
    // packet arrives from the network.
    std::vector<float> silence(m_impl->actualBufferSize, 0.0f);
    m_impl->playoutBuffer.write(silence.data(), silence.size());
